defmodule Exa.Dot.DotReader do
  @moduledoc """
  ...
  @dialyzer {:nowarn_function, from_dot: 1}
  @spec from_dot(String.t()) :: {D.graph(), D.graph_attrs()} | {:error, any()}
  def from_dot(filename) when is_nonempty_string(filename) do
  ...
  @typep context() :: {D.vert(), D.aliases(), gstack(), D.graph_attrs()}
  ...
  @spec parse([tok()]) :: {D.graph(), D.aliases(), D.graph_attrs()}
  defp parse([:digraph, gname, :open_brace | toks]) do
    # return the graph name?
    parse(toks, [], {1, %{}, [to_string(gname)], %{}})
  end

  @spec parse([tok()], D.graph(), context()) :: {D.graph(), D.aliases(), D.graph_attrs()}
  ...
  @spec id(charlist(), context()) :: {D.vert(), context()}
  ...
  defp id([c | _] = cs, {n, als, gstack, gattrs} = ctx) when is_alpha(c) or c == ?_ do
    name = to_string(cs)
